...
impl DoubleRocksdbBlobStore {
	...
	pub const COL_BLOB_BY_HASH_BLOCK: u32 = 5;
...
impl StorageApiT for DoubleRocksdbBlobStore {
	...
	fn get_blob_info_by_block(
		...
		match self.db_meta.get(Self::COL_BLOB_BY_HASH_BLOCK, &key)? {
			Some(bytes) => {
	...
	fn list_blob_infos_by_block(&self, block_hash: &BlockHash) -> Result<Vec<BlobInfo>> {
		...
		let iter = self
			.db_meta
			.iter_with_prefix(Self::COL_BLOB_BY_BLOCK, &prefix);
		let mut out = Vec::new();
		for kv in iter {
			let (_k, v) = kv?;
			let mut s = v.as_slice();
	...
	fn append_pending_blob_info(&self, block_hash: &BlockHash, blob_info: &BlobInfo) -> Result<()> {
		...
		let existing = self.db_meta.get(Self::COL_BLOB_PENDING_BY_BLOCK, &key)?;
