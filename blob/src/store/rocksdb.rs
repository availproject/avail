...
use crate::{
	types::{Blob, BlobEvalProof, BlobHash, BlobInfo, BlobMetadata, BlockHash, OwnershipEntry},
...
impl RocksdbBlobStore {
	...
	pub const COL_BLOB_EVAL_PROOF: u32 = 9;
...
impl StorageApiT for RocksdbBlobStore {
		...
		self.db
			...
			.map(|bytes| {
				...
				BlobEvalProof::decode(&mut slice)
					.map_err(|_| anyhow!("failed to decode blob eval proof from the store"))
			})
			.transpose()
	}
	...
	fn clean_blobs_info(&self, hashes: &Vec<BlobHash>) -> Result<()> {
		...
		for hash in hashes {
			...
			// remove blob eval proofs
			tx.delete_prefix(Self::COL_BLOB_EVAL_PROOF, &blob_eval_proof_key_prefix(hash));
