...
use crate::{
	...
	workers::{
		chain_api,
		common::{read_pallet_call_index, Timer},
		macros::profile,
	},
};
...
impl Worker {
	...
	async fn index_new_blocks(self) {
		let mut logger = Logger::new("Index New Blocks Worker".into(), self.cli.logging_interval);

		let mut current_block_hash = H256::default();
		loop {
			let (block_height, block_hash) = self.wait_for_new_best_block(current_block_hash).await;
			current_block_hash = block_hash;
			let now = Instant::now();

			let block: Result<(Vec<OpaqueExtrinsic>, H256), FetchBlockError> =
				fetch_block_body(&self.client, block_height).await;
			let Ok((opaques, block_hash)) = block else {
				continue;
			};
			let block = prepare_block(opaques, block_hash, block_height, false);
	...
	pub async fn index_finalized_blocks(self) {
		...
		loop {
			...
			let block = prepare_block(opaques, block_hash, next_block_height, true);
	...
	async fn index_old_blocks(&self, logger: &Logger) -> u32 {
		...
		while height != 0 && limit != 0 {
			...
			let block = prepare_block(opaques, block_hash, height, true);
