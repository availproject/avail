...
async function sendTx(
  ...
  try {
    ...
    const res = await submit.signAndSend(
      sender, // sender
      options, // options
...
async function get(api: any, block_hash: H256, extrinsic_hash: H256) {
  ...
  extrinsics.forEach(async (ex: any, index: number) => {
    console.log("printing appId",ex.appId.toHuman());
    console.log(index, ex);
    if (extrinsic_hash == ex.hash.toHex()) {
      console.log("printing method", ex.method.callIndex);
      console.log("printing index and ex", index, ex.toHuman());
      const {
      ...
      console.log(`\n 💡 DATA_SUBMITTED : ${str}`);
      console.log(
        `${section}.${method}(${args.map((a: any) => a.toString()).join(", ")})`
      );
    }
  });
  //process.exit(0);
...
let block = async (hash: H256, api: ApiPromise) => {
  ...
  console.log(
    `💡Tx included in Block Number: ${block_num} with hash ${hash}\n`
  );
};
