...
const stakingBond = new Promise<[H256, H256]>((res, _) => {
  api.tx.staking.bond(oneAvl, "Staked").signAndSend(
    bob,
...
const [blockHash, txHash] = await stakingBond;
console.log(`Finalized block hash ${blockHash} and tx hash ${txHash}`);

// Find and print out Staking Bond transaction
