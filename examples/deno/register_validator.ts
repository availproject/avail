...
const config: Config = JSON.parse(Deno.readTextFileSync("./register_validator.json"));

if (config.stash_account_seed_hex == undefined || config.bond_amount == undefined || config.endpoint == undefined || config.commission == undefined) {
...
if (config.commission < 0 || config.commission > 100) {
    console.log("commission cannot be less than 0. commission cannot be more than 100");
    Deno.exit(0);
}
...
const stash = new Keyring({type: 'sr25519'}).addFromUri(config.stash_account_seed_hex);
...
const bond_value = new BN(config.bond_amount.toString().concat("000000000000000000"));
...
console.log(`Authority Discovery Key (derived from Session Keys): ${authority_discovery_key}`);
if (already_bonded) {
    console.log("Stash is already bonded. No new staking.bond call will be executed");
}
...
const tx = new Promise<string | undefined>((res, _) => {
    ...
    api.tx.utility.batchAll(calls).signAndSend(stash, (result: ISubmittableResult) => {
