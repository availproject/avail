...
export class BlockInclusionStage implements Stage {
	...
	async run(task: Task): Promise<void> {
		console.log(
			`Task ${task.name}: Waiting block inclusion ${this.blockTarget}`,
		);
...
export class DataSubmissionStage implements Stage {
	...
	async run(task: Task): Promise<void> {
		...
		let nonce = (await task.api.rpc.system.accountNextIndex(this.account.address))
			.toNumber();
		const txs = [];
		for (let i = 0; i < task.txCount; ++i) {
			txs.push(
				task.api.tx.dataAvailability.submitData(task.data[i].toString())
					.signAndSend(this.account, { nonce: nonce }),
			);
			nonce += 1;
		}
...
export class BlockFinalizationStage implements Stage {
	...
	async run(task: Task): Promise<void> {
		console.log(
			`Task ${task.name}: Waiting Block Finalization ${this.blockTarget}`,
		);
		task.finalizedBlockHash = await waitForBlockFinalization(
			task.api,
			this.blockTarget,
		);
	}
