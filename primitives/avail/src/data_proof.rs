	...
	H: PartialEq + Eq + AsRef<[u8]>,
{
	...
	fn try_from(merkle_proof: &MerkleProof<H, T>) -> Result<Self, Self::Error> {
		...
		let root = <[u8; 32]>::try_from(merkle_proof.root.as_ref())
			...
			.into();
		let leaf = sp_io::hashing::keccak_256(merkle_proof.leaf.as_ref()).into();
...
mod test {
	...
	fn expected_data_proof_1() -> Result<DataProof, DataProofTryFromError> {
		Ok(DataProof {
			...
			number_of_leaves: 7,
			leaf_index: 1,
			leaf: sp_io::hashing::keccak_256(H512::repeat_byte(1).as_bytes()).into(),
		})
	}
	...
	fn expected_data_proof_0() -> Result<DataProof, DataProofTryFromError> {
		Ok(DataProof {
			...
			number_of_leaves: 7,
			leaf_index: 0,
			leaf: sp_io::hashing::keccak_256(H512::repeat_byte(0).as_bytes()).into(),
		})
	}
	...
	fn expected_data_proof_6() -> Result<DataProof, DataProofTryFromError> {
		Ok(DataProof {
			...
			number_of_leaves: 7,
			leaf_index: 6,
			leaf: sp_io::hashing::keccak_256(H512::repeat_byte(6).as_bytes()).into(),
		})
	}
