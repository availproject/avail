...
func transfer(api *sdk.SubstrateAPI, senderSeed string, receiver string, amount uint64) error {
	...
	latestBlock, err := rpc.GetAvailBlockLatest(api.Client)
	if err != nil {
		panic(fmt.Sprintf("cannot get Latest Block: %v", err))
	}
	var e extrinsic.ExtrinsicEra
	e.IsMortalEra = true
	...
	e.AsMortalEra = extrinsic.MortalEra{First: types.U64(128), Second: types.U64(second)}
