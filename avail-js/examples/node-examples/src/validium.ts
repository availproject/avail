...
const main = async () => {
    try {
        ...
        if (result.isFinalized) {
            console.log(`DA transaction in finalized block: ${result.status.asFinalized}, transaction index: ${result.txIndex}`);
        }
        ...
        while (true) {
            ...
            if (lastCommittedBlock >= blockNumber) {
                ...
                const proofResponse = await fetch(config.bridgeApi + "/eth/proof/" + result.status.asFinalized + "?index=" + result.txIndex);
