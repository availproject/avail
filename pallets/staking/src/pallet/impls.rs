...
impl<T: Config> Pallet<T> {
	...
	fn check_payees() -> Result<(), TryRuntimeError> {
		for (stash, _) in Bonded::<T>::iter() {
				...
				Payee::<T>::get(&stash).is_some(),
				"bonded ledger does not have payee set"
			);
		}
		...
		Bonded::<T>::iter()
			.map(|(stash, ctrl)| {
				...
				let raw = Ledger::<T>::get(stash).unwrap_or_else(|| {
					...
					raw.controller.is_none(),
					"raw storage controller should be None"
				);
		...
		exposures
			...
			.map(|(validator, metadata)| {
				let actual_overview = ErasStakersOverview::<T>::get(era, validator);
					...
					actual_overview.is_some(),
					"No overview found for a paged exposure"
				);
