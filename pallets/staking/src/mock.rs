...
use frame_system::{
	...
	test_utils::TestRandomness, EnsureRoot, EnsureSignedBy,
};
use sp_core::H256;
use sp_io;
use sp_runtime::{
...
pub const INIT_TIMESTAMP: u32 = 30_000;
pub const BLOCK_TIME: u32 = 1000;
	...
	fn build(self) -> sp_io::TestExternalities {
		...
		if self.initialize_first_session {
			...
			ext.execute_with(|| {
				...
				Timestamp::set_timestamp(INIT_TIMESTAMP.into());
			});
		}
...
pub(crate) fn run_to_block(n: BlockNumber) {
	...
	for b in (System::block_number() + 1)..=n {
		...
		Timestamp::set_timestamp((System::block_number() * BLOCK_TIME + INIT_TIMESTAMP).into());
		if b != n {
			Staking::on_finalize(System::block_number());
		}
	}
...
pub(crate) fn time_per_session() -> u32 {
...
/// timestamp after on_initialize, so the timestamp is always one block old.
pub(crate) fn time_per_era() -> u32 {
	time_per_session() * SessionsPerEra::get() as u32
}

/// Time that will be calculated for the reward per era.
pub(crate) fn reward_time_per_era() -> u32 {
