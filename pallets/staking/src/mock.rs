...
pub(crate) fn run_to_block(n: BlockNumber) {
	...
	for b in (System::block_number() + 1)..=n {
		...
		let ts: u64 = block_number * BLOCK_TIME + INIT_TIMESTAMP;
...
pub(crate) fn start_session(session_index: SessionIndex) {
	let end: u64 = if Offset::get().is_zero() {
		(session_index as u64) * (Period::get() as u64)
	} else {
		(Offset::get() as u64) + (session_index.saturating_sub(1) as u64) * (Period::get() as u64)
	};
	run_to_block(end.try_into().unwrap_or(0u32));
