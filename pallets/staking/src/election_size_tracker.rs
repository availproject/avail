...
mod tests {
	...
	pub fn election_size_tracker_works() {
		...
		let mut size_tracker = StaticTracker::<Staking>::default();
		let voter_bounds = ElectionBoundsBuilder::default()
			.voters_size(1_50.into())
			.build()
			.voters;
		...
		assert!(size_tracker
			.try_register_voter(&voter, &voter_bounds)
			.is_ok());
		voters.push(voter);
	...
	#[test]
	fn len_prefix_works() {
		let length_samples = vec![
			0usize, 1, 62, 63, 64, 16383, 16384, 16385, 1073741822, 1073741823, 1073741824,
		];
		...
		for s in length_samples {
			...
			assert_eq!(
				...
				StaticTracker::<Staking>::length_prefix(s) + s
			);
		}
	}
}
