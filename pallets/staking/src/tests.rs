...
fn rewards_should_work() {
	ExtBuilder::default()
		...
		.build_and_execute(|| {
			...
			let total_payout_0 = current_total_payout_for_duration(reward_time_per_era() as u64);
			let maximum_payout = maximum_payout_for_duration(reward_time_per_era() as u64);
			...
			let total_payout_1 = current_total_payout_for_duration(reward_time_per_era() as u64);
...
fn reward_destination_works() {
	...
	ExtBuilder::default().nominate(false).build_and_execute(|| {
		...
		// Compute total payout now for whole duration as other parameter won't change
		let total_payout_2 = current_total_payout_for_duration(reward_time_per_era() as u64);
		Pallet::<Test>::reward_by_ids(vec![(11, 1)]);
...
fn test_nominators_over_max_exposure_page_size_are_rewarded() {
	ExtBuilder::default().build_and_execute(|| {
		...
		let _ = current_total_payout_for_duration(reward_time_per_era() as u64);
...
fn test_multi_page_payout_stakers_by_page() {
	...
	ExtBuilder::default()
		...
		.build_and_execute(|| {
			...
			// compute and ensure the reward amount is greater than zero.
			let payout = current_total_payout_for_duration(reward_time_per_era() as u64);
			mock::start_active_era(2);
			...
			for era in 0..15 {
				assert_eq!(
					Staking::claimed_rewards(era, &11),
			...
			assert_eq!(
				Staking::claimed_rewards(first_claimable_reward_era, &11),
				vec![0]
			);
