...
use crate::{
	Broadcasters, ConfigurationStorage, DisableLC, Error, Event, ExecutionStateRoots,
	FunctionInput, FunctionOutput, FunctionProof, Head, Headers, MessageStatus, SourceChainFrozen,
...
#[test]
fn test_full_fill_step_call_lc_disabled() {
	new_test_ext().execute_with(|| {
		let slot = 7634942;
		SyncCommitteePoseidons::<Test>::insert(
			931,
		...
		ConfigurationStorage::<Test>::set(Configuration {
			slots_per_period: 8192,
			finality_threshold: 342, // max finality
		});

		DisableLC::<Test>::set(true);
		...
		assert_err!(result, Error::<Test>::LCDisabled);
	});
}
