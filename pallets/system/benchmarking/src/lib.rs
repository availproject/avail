...
pub trait Config: frame_system::Config {
	...
	///
	/// Default is `../res/da_runtime.compact.compressed.wasm`.
	fn prepare_set_code_data() -> Vec<u8> {
		include_bytes!("../res/da_runtime.compact.compressed.wasm").to_vec()
	}
...
mod benchmarks {
	...
	#[benchmark(extra)]
	fn header_extension_builder() -> Result<(), BenchmarkError> {
