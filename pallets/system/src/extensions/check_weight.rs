...
mod tests {
	...
	fn normal_extrinsic_limited_by_maximum_extrinsic_weight() {
		new_test_ext().execute_with(|| {
			let max = DispatchInfo {
				call_weight: block_weights()
					.get(DispatchClass::Normal)
					.max_extrinsic
					.unwrap() + Weight::from_parts(1, 0),
	...
	fn full_block_with_normal_and_operational() {
		new_test_ext().execute_with(|| {
			...
			let max_normal = DispatchInfo {
				call_weight: Weight::from_parts(753, 0),
				..Default::default()
			};
			...
			assert_eq!(
				...
				block_weight_limit().set_proof_size(0)
			);
	...
	fn signed_ext_check_weight_works_normal_tx() {
		new_test_ext().execute_with(|| {
			...
			let medium = DispatchInfo {
				call_weight: normal_limit - base_extrinsic,
				..Default::default()
			};
	...
	fn signed_ext_check_weight_refund_works() {
		new_test_ext().execute_with(|| {
			...
			BlockWeight::<Test>::mutate(|current_weight| {
				...
				current_weight.set(
					Weight::from_parts(256, 0) - base_extrinsic,
	...
	fn no_max_total_should_still_be_limited_by_max_block() {
		...
		assert_eq!(
			maximum_weight.max_block,
			all_weight.total().set_proof_size(u64::MAX)
		);
