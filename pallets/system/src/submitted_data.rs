...
mod test {
	...
	fn test_data_proof_with_skipped_tx() {
		...
		let tx2_data: String = String::new();
		...
		if let Some(da_proof) = calls_proof::<String, _, _>(submitted_data.clone().into_iter(), 0) {
			assert_eq!(da_proof.root.to_string(), "0x0f1d…c02d");
			assert_eq!(da_proof.proof.len(), 2);
			assert_eq!(da_proof.proof[0].to_string(), "0xc89e…8bc6");
			assert_eq!(da_proof.proof[1].to_string(), "0xad7c…14a5");
			assert_eq!(da_proof.number_of_leaves, 3);
		} else {
			panic!("Proof not generated for the transaction index 0!");
		}

		// second item should be skipped and proof should not be generated
		if let None = calls_proof::<String, _, _>(submitted_data.clone().into_iter(), 1) {
			// all good
		} else {
			panic!("Proof should not be generated generated!");
		}

		if let Some(da_proof) = calls_proof::<String, _, _>(submitted_data.clone().into_iter(), 2) {
			assert_eq!(da_proof.root.to_string(), "0x0f1d…c02d");
			assert_eq!(da_proof.proof.len(), 2);
			assert_eq!(da_proof.proof[0].to_string(), "0x0448…116d");
			assert_eq!(da_proof.proof[1].to_string(), "0xad7c…14a5");
			assert_eq!(da_proof.number_of_leaves, 3);
		} else {
		...
		if let Some(da_proof) = calls_proof::<String, _, _>(submitted_data.clone().into_iter(), 3) {
			...
			assert_eq!(da_proof.proof[0].to_string(), "0x0b4a…fef2");
			assert_eq!(da_proof.number_of_leaves, 3);
		} else {
			panic!("Proof not generated for the transaction index 3!");
		}

		// submit index that does not exists
		if let None = calls_proof::<String, _, _>(submitted_data.clone().into_iter(), 15) {
			// all good
		} else {
			panic!("Proof should not be generated generated!");
		}
